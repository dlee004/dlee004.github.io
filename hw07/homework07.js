...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            cone.updateNormals();
            shadingMode = 'SMOOTH';
            updateText(textOverlay3, "shading mode: " + shadingMode + gpMode);
            render();
        }
        else if (event.key == 'f') {
...
function render() {
    ...
    let shaderToUse = (currentShader === 'PHONG') ? phongshader : gouraudshader;

    shaderToUse.use();
    shaderToUse.setMat4('u_model', modelMatrix);
    shaderToUse.setMat4('u_view', viewMatrix);
    shaderToUse.setVec3('u_viewPos', cameraPos);
    cone.draw(shaderToUse);
...
async function main() {
    try {
        ...
        textOverlay3 = setupText(canvas, "shading mode: " + shadingMode + gpMode, 3);
